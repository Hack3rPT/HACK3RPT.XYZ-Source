...
void GetConfigMassive()
{
	...
	szPath1 = std::string(path) + "\\hack3rpt.xyz\\*";
...
void RenderInterface() {
	...
	if (is_renderer_active)
	{
...

		{
			...
			Begin("HACK3RPT.XYZ | PRIVATE VERSION", &is_renderer_active, ImVec2(700, 500), 1.f, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize);
			...
			if (rage)
			{
				...
				Checkbox(("Auto Crouch (Beta)*"), &g_Options.Ragebot.AutoCrouch);
				Checkbox(("No Recoil"), &g_Options.Ragebot.AntiRecoil);
				Checkbox(("MultiPoint (Beta)*"), &g_Options.Ragebot.Multipoint);
				Spacing();
				Combo(("Resolver"), &g_Options.Ragebot.Resolver, "Off\0\rSimple\0\rBrute Memes\0\rPrediction\0\rSmart\0\0", -1);
			...
			if (visuals)
			{
				...
				Spacing();
				Checkbox("LBY Indicator", &g_Options.Visuals.LBYIndicator);
				EndChild();
			...
			if (mis)
			{
				...
				Spacing();
				Text("Wrong Section :");
				Checkbox(("Auto Pistol"), &g_Options.Ragebot.AutoPistol);
				Spacing();
				EndChild();


				BeginChild(10, ImVec2(-1, 108), true, 0);
				TextColored(ImVec4(g_Options.Colors.MenuTheme[0], g_Options.Colors.MenuTheme[1], g_Options.Colors.MenuTheme[2], 1.f), "Thirdperson");
				Separator();
				Spacing();
				Hotkey("Key", &g_Options.Visuals.TPKey);
				Combo("Angles", &g_Options.Visuals.TPAng, "Real\0\rFake");
				EndChild();
				NextColumn();
			...
			if (skins)
			{
				...
				Spacing();
				Columns(2, 0, false);
				Combo("Knife Model", &g_Options.Skinchanger.knifemodel, "Default\0\rBayonet\0\rFlip\0\rGut\0\rKarambit\0\rM9 Bayonet\0\rHuntsman\0\rFalshion\0\rBowie\0\rButerfly\0\rShadow Daggers\0\0", -1);
				NextColumn();
