...
void CConfig::Save()
{
	...
	{
		char szCmd[256];
		sprintf(szCmd, "\\hack3rpt.xyz\\%s.cfg", g_Options.ConfigName);

		folder = std::string(path) + "\\hack3rpt.xyz\\";
...
void CStatConfig::SaveSTT()
{
	...
	{
		folder = std::string(path) + ("\\hack3rpt.xyz\\");
		file = std::string(path) + ("\\hack3rpt.xyz\\") + std::string("Stattrack") + XorStr(".noedit");
	}
