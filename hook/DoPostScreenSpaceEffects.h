...
bool _fastcall hkDoPostScreenSpaceEffects(void* ecx, void* edx, CViewSetup* pSetup)
{
			...
			if (local)
			{
				for (int i = 0; i < g_GlowObjManager->m_GlowObjectDefinitions.Count(); ++i)
				{
					...
					case 35:
						...
						{
							if (glowEnt->getEnt()->GetTeamNum() == local->GetTeamNum() && g_Options.Visuals.TeamESP)
							...
							else if (glowEnt->getEnt()->GetTeamNum() != local->GetTeamNum() && g_Options.Visuals.Box)
	...
	catch (std::exception(SSE))
	{
		char buf[100];
		sprintf(buf, "DoPostScreenSpaceEffects: %s", SSE.what());
		MessageBox(NULL, buf, "Whoops, hack3rpt.xyz Crashed", MB_OK);
	}
}