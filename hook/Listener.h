			...
			void on_fire_event(IGameEvent* event, bool drawing)
			{
				...
				if (!drawing)
				{
					...
					if (!strcmp(event->GetName(), "item_purchase"))
					{
						...
						if (player && local && g_Engine->GetPlayerInfo(player_index, &pinfo))
						{
							...
							if (g_Options.Visuals.EventLog) {
								...
								{
									g_CVar->ConsoleColorPrintf(Color(g_Options.Colors.MenuTheme[0] * 255, g_Options.Colors.MenuTheme[1] * 255, g_Options.Colors.MenuTheme[2] * 255), "[hack3rpt.xyz] ");
					...
					if (!strcmp(event->GetName(), "player_death"))
					{
						...
						if (died_ == local)
						{
							...
							case 1:
								...
								break;
							case 2:
								g_Engine->ClientCmd_Unrestricted(u8"say Como é que te atreves a matar um usuario do hack3rpt.xyz ?");
								break;
							}
						}
						if (killer_ == local)
						{
							...
							{
							case 1:
								ss << "say " << info.name << ", you got rekt by hack3rpt.xyz, using a weapon named " << event->GetString("weapon") << ".";
								g_Engine->ClientCmd_Unrestricted(ss.str().c_str());
								break;
							case 2:
								ss1 << "say mpt " << info.name << ", acabaste de ser destruido por hack3rpt.xyz, usando " << event->GetString("weapon") << ".";
					...
					if (!strcmp(event->GetName(), "bomb_abortdefuse"))
					{
						...
						{
						case 1:
							ss << "say " << info.name << " aborted the bomb defusal like a fucking pussy!!! MAYBE HE NEEDS SOME GOOD CHEAT ;)"; break;
						case 2:
					...
					if (!strcmp(event->GetName(), "round_freeze_end"))
					{
						...
						{
						case 1:
							g_Engine->ClientCmd_Unrestricted("say What a wonderful day to use hack3rpt.xyz"); break;
						case 2:
							g_Engine->ClientCmd_Unrestricted("say Que belo dia para usar hack3rpt.xyz"); break;
						}
					}
