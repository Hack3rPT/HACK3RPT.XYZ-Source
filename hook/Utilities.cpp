...
void  U::Log(const char *fmt, int type)
{
	...
	if (logBuf[0] != '\0')
	{
		...
		switch (type)
		{
		case LOG_NORMAL: SetConsoleColor(FOREGROUND_WHITE); break;
		case LOG_ADVICE: SetConsoleColor(FOREGROUND_INTENSE_GREEN); break;
		case LOG_WARNING: SetConsoleColor(FOREGROUND_INTENSE_YELLOW); break;
		case LOG_FATAL: SetConsoleColor(FOREGROUND_INTENSE_RED); break;
		}
		printf(": %s\n", logBuf);
	}
...
float U::get_fraction_props_only(Vector start, Vector end)
{
	Ray_t ray;
	CGameTrace tr;
	CTraceFilter filter;
