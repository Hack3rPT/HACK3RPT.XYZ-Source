...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(1, 0), pos + ImVec2(1, 0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(33, 6, 4, 45));        // Shadow
			g.OverlayDrawList.AddImage(tex_id, pos + ImVec2(2, 0), pos + ImVec2(2, 0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(33, 6, 4, 45));        // Shadow
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(51, 51, 51, 142));       // grey (almost black) border
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(204, 0, 0, 162)); // ( not anymore ) grey* fill
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
	...
	ImGui::PushStyleColor(ImGuiCol_ChildWindowBg, style.Colors[Massashook_LisboxBg]);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	draw->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_ButtonActive), style.FrameRounding, -1, 1.5f);
	if (held)
		draw->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_WindowBg));
	else
		draw->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_ChildWindowBg), GetColorU32(ImGuiCol_ChildWindowBg), GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_WindowBg));
...
bool ImGui::ToggleButton(const char* label, bool* v, const ImVec2& size_arg)
{
	...
	if (*v)
	{
		draw->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_ButtonActive), 5.f, 0, 1.5f);
		draw->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_ButtonActive), GetColorU32(ImGuiCol_ButtonActive), GetColorU32(ImGuiCol_ChildWindowBg), GetColorU32(ImGuiCol_ChildWindowBg));
	}
	else
	{
		draw->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Button), 5.f, 0, 1.f);
		draw->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImGuiCol_ChildWindowBg), GetColorU32(ImGuiCol_ChildWindowBg), GetColorU32(ImGuiCol_Button), GetColorU32(ImGuiCol_Button));
	}
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color)
{
	...
	// Draw frame
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 2.5f);
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBgActive));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 2.5f);
	if (*v)
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImGuiCol_TitleBgActive), style.FrameRounding);
	}
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	if (active)
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddCircleFilled(center, radius - pad, GetColorU32(ImGuiCol_TitleBg), 16);
	}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (!is_multiline)
	{
		drawlist->AddRectFilled(frame_bb.Max, frame_bb.Max, GetColorU32(ImGuiCol_ChildWindowBg), 0, 0);
	}
	...
	if (g.ActiveId == id || is_currently_scrolling)
	{
		...
		draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 1.5f);
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 1.5f);

	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(ImGuiCol_TitleBgActive), true, 00.f); // FIXME-ROUNDING
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (hovered || selected)
	{
		if (selected)
		{
			window->DrawList->AddRect(bb.Min, bb_with_spacing.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 2.f);
		}
		else if (hovered)
		{
			window->DrawList->AddRect(bb.Min, bb_with_spacing.Max, GetColorU32(ImGuiCol_TitleBgActive), 0, 0, 1.f);
		}
	}
